...
int main()
{
    ...
    cout << "\n \\--> Field1: " << Field1 << "\n"; 
    assert(Field1.getValue() == random1);

    Galois Field2(random2);
    cout << "\n-> Second random value (negative):";
    cout << "\n \\--> Field2: " << Field2 << "\n"; 
    assert(Field2.getValue() == (FIELD_ORDER + random2) % FIELD_ORDER);
    ...
    logicExpected = 
        ((FIELD_ORDER + random1) % FIELD_ORDER) < 
        ((FIELD_ORDER + random2) % FIELD_ORDER); 
    ...
    assert(testInteger == newField.getValue());
